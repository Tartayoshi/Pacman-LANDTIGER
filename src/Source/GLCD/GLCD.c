...
/*********************************************************************
*
*Calibration Parameter for GetPoint, solve some isssues
*
*********************************************************************/
char bgr_lcd = 0; //calibration needed, used for some display issues
...
//					return  LCD_BGR2RGB( dummy );
        ...
        default:	/* 0x9320 0x9325 0x9328 0x9331 0x5408 0x1505 0x0505 0x9919 */
             dummy = LCD_ReadData();   /* Empty read */
             dummy = LCD_ReadData(); 	
				if(bgr_lcd == 1){ //calibrated output
					return  LCD_BGR2RGB( dummy );
				}else{
						return dummy;
				}
...
/******************************************************************************
* Function Name  : LCD_GCalibrateGetPoint
* Description    : Set bgr_lcd parameter accordigly to a write and read on the screen
* Input          : None
* Output         : None
* Return         : None
* Attention		 : None
...
void LCD_CalibrateGetPoint(void){
	int rgb = 0; //some display hve issue with get point
	int i;
	uint16_t volatile color; //for debug, used in the variable Watch list
	
	for(i = 0; i < 10; i++){ //why ten? because some board for some reason don't always show the correct value
													//while using the getPoint.
		#ifdef SIMULATOR //getPoin on simulator doesn't like small values
		LCD_SetPoint(120, 160, Blue);
		color = LCD_GetPoint(120, 160);
		#else
		LCD_SetPoint(0, 0, Blue); //draw a point every time to be sure that it will present, solve issue with some screen
		color = LCD_GetPoint(0, 0);
		#endif
		if(color == Blue){ //if is not red and is blue, the screen use RGB format, else is BGR
			rgb++;
		}
	}
	
	if(rgb > 4){ //If half of the read are for RGB the parametr will tell to the GetPoint that the screen is RGB 
		bgr_lcd = 0;
		//LCD_Clear(Green);
	}else{      // else the screen is bgr
		bgr_lcd = 1;
		//LCD_Clear(Blue);
	}
